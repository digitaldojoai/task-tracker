...
| --------- | -------------------------------------------------- | --------------------------- |
| Backend   | Python 3.13, FastAPI, Pydantic v2, Uvicorn          | `backend/app/`              |
| Storage   | In-memory dict — no database, resets on restart     | `backend/app/storage.py`    |
| Tests     | pytest + httpx (FastAPI `TestClient`)               | `backend/tests/`            |
...
Note on layout: the course brief refers to `app/` and `tests/` at the repository
root. In this repo they live under `backend/` (`backend/app/`, `backend/tests/`).
Nothing else about the structure differs.
...
`app.main:app` is an import path. Uvicorn must be started from `backend/`, or the
`app` package is not importable and it fails with `ModuleNotFoundError`.
...
1. **Read before you write.** Read the file you intend to change, plus its
   ...
   name.
2. **Check the docs against the code.** `README.md`, `backend/README.md`, and
...
## Rules for changing `backend/app/` and `frontend/`
...
- `backend/.env` is gitignored and must stay untracked. `backend/.env.example`
